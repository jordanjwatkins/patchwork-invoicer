...
<div id="invoice-info">
	...
	<div><strong>Invoice Date:</strong> <?php echo date("F d, Y", $invoice->invoice_date); ?></div>
	<div><strong>Invoice Number:</strong> <?php echo $invoice->client->client_number . $invoice->invoice_number; ?></div>
</div>
<header>
...
<?php foreach ($invoice->projects as $project) { ?>
	...
	<div class="project-amounts">
		<?php
			if (!empty($project->hourly_hours)) {
				foreach ($project->hourly_hours as $hourly => $hours) { ?>
...
<a class="back" href="<?php echo base_url('invoices'); ?>">< Back to Invoices</button>